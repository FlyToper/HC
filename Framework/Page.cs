...
namespace 基于云的Web管理系统.Framework
{
    public class Page
    {
        ...
        /// <param name="totalCount">总的条数</param>
        /// <param name="searchType">搜索类型：搜索全部、精确搜索</param>
        /// <param name="cname">分类名字</param>
        /// <param name="ltime">分割时间</param>
        /// <param name="name">发布人</param>
        /// <returns></returns>
        public static string ShowForumSearchNavigate(string searchType,int currentPage, int pageSize, int totalCount,  string cname, string ltime, string name)
        {
            ...
            if (totalPages > 1)
            {
                //if (currentPage != 1)
                {//处理首页连接
                    output.AppendFormat("<a class='pageLink' href='javascript:void(0);' onclick='Search({0},{1},{2},{3},{4},{5})'>首页</a> ", searchType, 1, pageSize, cname, ltime, name);
                }
                if (currentPage > 1)
                {//处理上一页的连接

                    output.AppendFormat("<a class='pageLink' href='javascript:void(0);' onclick='Search({0},{1},{2},{3},{4},{5})'>上一页</a> ", searchType, currentPage - 1, pageSize, cname, ltime, name);
                }
                else
                {
                    // output.Append("<span class='pageLink'>上一页</span>");
                ...
                {//一共最多显示10个页码，前面5个，后面5个
                    if ((currentPage + i - currint) >= 1 && (currentPage + i - currint) <= totalPages)
                    {
                        ...
                        {//一般页处理
                            ...
                            output.AppendFormat("<a class='pageLink'  href='javascript:void(0);' onclick='Search({0},{1},{2},{3},{4},{5})'>{6}</a> ", searchType, currentPage + i - currint, pageSize, cname, ltime, name, currentPage + i - currint);
                        }
                    }
                    output.Append(" ");
                }
                ...
                {//处理下一页的链接
                    ...
                    output.AppendFormat("<a class='pageLink' href='javascript:void(0);' onclick='Search({0},{1},{2},{3},{4},{5})'>下一页</a> ", searchType, currentPage + 1, pageSize, cname, ltime, name);
                }
                else
                {
                    //output.Append("<span class='pageLink'>下一页</span>");
                ...
                if (currentPage != totalPages)
                {
                    ...
                    output.AppendFormat("<a class='pageLink' href='javascript:void(0);' onclick='Search({0},{1},{2},{3},{4},{5})'>末页</a> ", searchType, totalPages, pageSize, cname, ltime, name);
                }
                output.Append(" ");
            }
            output.AppendFormat("第{0}页 / 共{1}页", currentPage, totalPages);//这个统计加不加都行
